...
LifeUnknownState LifeUnknownState::StepMaintaining(const LifeStableState &stable) const {
  ...
  LifeState useRefined = ~(unknown & ~unknownStable).ZOI();
  ...
  for (int i = 0; i < N; i++) {
    ...
    uint64_t naive_next_unknown_stable = 0;
...
#include "bitslicing/unknown_step_refined_m.hpp"
    ...
    result.state[i] = (naive_next_on & ~useRefined[i]) | (next_on & useRefined[i]);
    result.unknown[i] = (naive_next_unknown & ~useRefined[i]) | (next_unknown & useRefined[i]);
    result.unknownStable[i] = (next_unknown_stable & useRefined[i]);
  }
